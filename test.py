...
def RSSscrape(start, cik, type):
    feed_url = url + '&start=' + str(start) + '&CIK=' + str(cik) + '&type=' + str(type) + '&count=100&output=atom'
    data = []
    res = []
...
def lambda_handler(event, context):
    ...
    try:
        new = json.loads(json.dumps(RSSscrape('','','')))
        for m in new:
            if m not in old:
        ...
        _8k_data = filter(lambda updated_new: updated_new['formtype']=='8-K', updated_new)
        _10k_data = filter(lambda updated_new: updated_new['formtype']=='10-K', updated_new)
        # sqs.send_message(
        #     QueueUrl= sqs_queue,
        #     DelaySeconds=0,
        #     MessageAttributes={
        ...
        #         }
        #     },
        #     MessageBody= json.dumps(updated_new))
